...
<div class="reference"><div class="kicker">Payment record</div><p>Processed through {{ $details['provider'] }}</p><p>Reference: <strong>{{ $details['payment_reference'] }}</strong></p><p class="muted">Confirmed {{ $receipt->issued_at->format('d M Y, H:i') }} {{ $receipt->issued_at->timezoneName }}</p></div>
