...
<section class="contact-layout section-shell">
  <div class="contact-copy"><span class="kicker">Start a project</span><h1>Tell us what you’re trying to <em>move forward.</em></h1><p>Share the challenge, opportunity, or rough idea. We will help identify the clearest next step.</p><div class="contact-detail"><span>Email</span><a href="mailto:{{ config('variables.email') }}">{{ config('variables.email') }}</a><a href="mailto:{{ config('variables.emailSecondary') }}">{{ config('variables.emailSecondary') }}</a></div><div class="contact-detail"><span>Call</span><a href="tel:{{ config('variables.phone') }}">{{ config('variables.phoneDisplay') }}</a><a href="tel:{{ config('variables.phoneSecondary') }}">{{ config('variables.phoneSecondaryDisplay') }}</a></div><div class="contact-detail"><span>Based in</span><strong>{{ config('variables.location') }}</strong></div><a href="{{ config('variables.whatsappUrl') }}" target="_blank" rel="noopener" class="button whatsapp-button"><i class="bx bxl-whatsapp"></i> Chat on WhatsApp</a></div>
