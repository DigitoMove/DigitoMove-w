...
Route::middleware(['auth', 'admin'])->group(function () {
    Route::resource('/admin/invoices', \App\Http\Controllers\Billing\AdminInvoiceController::class)->except('destroy')->names('admin.invoices');
    Route::post('/admin/invoices/{invoice}/issue', [\App\Http\Controllers\Billing\AdminInvoiceController::class, 'issue'])->name('admin.invoices.issue');
    Route::post('/admin/invoices/{invoice}/void', [\App\Http\Controllers\Billing\AdminInvoiceController::class, 'void'])->name('admin.invoices.void');
});
Route::get('/invoices/{token}', [\App\Http\Controllers\Billing\PublicInvoiceController::class, 'show'])->name('invoices.show');
Route::post('/invoices/{token}/pay', [\App\Http\Controllers\Billing\PublicInvoiceController::class, 'checkout'])->middleware('throttle:6,1')->name('invoices.pay');
Route::post('/invoices/{token}/refresh', [\App\Http\Controllers\Billing\PublicInvoiceController::class, 'refresh'])->middleware('throttle:6,1')->name('invoices.refresh');
